function navbar() {
  ...
  const links = ul({ class : 'navbar-nav'},
    li({ class : 'nav-item' },
      a({ class : 'nav-link', href : '/bridges-hs/overview/' }, 'Overview')
    ),
  ...
  const navbar = nav({ class : 'navbar navbar-expand navbar-custom' },
    div({ class : 'container' },
      a({ class : 'navbar-brand', href : '/bridges-hs/' },
        img({ src : '/bridges-hs/images/logo.svg', alt : 'BRIDGES Logo' })
      ),
      links
    )
  );
...
function assignmentPage(id) {
  ...
  //image and caption
  const source = '/bridges-hs/images/example-output/' + id + '.png'
  ...
  ids.forEach(id => {
    ...
    const card = div({ class : 'card assignment-card link' },
      div({ class : 'card-thumbnail' },
        img({ src : '/bridges-hs/images/example-output/' + id + '.png', alt : asst.name + ' Example Output' })
      ),
      div({ class : 'card-body' },
    ...
    card.onclick = () => {
      window.location = '/bridges-hs/assignment?name=' + id;
    };
